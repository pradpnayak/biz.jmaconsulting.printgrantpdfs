...
{php}
  ...
  $ovNames = array(
    'grant' => array(
      'grant_print_pdf' => ts('Grants - Print PDF template', array('escape' => 'sql')),
    ),
  );
...
{foreach from=$ovs key=vName item=title name=for_values}
      ...
      ('{$title}', {ts}'Grant Print PDF template'{/ts}, '', '{$html|escape:"quotes"}', @tpl_ovid_{$vName}, 1,          0),
      ('{$title}', {ts}'Grant Print PDF template'{/ts}, '', '{$html|escape:"quotes"}', @tpl_ovid_{$vName}, 0,          1) {if $smarty.foreach.for_groups.last and $smarty.foreach.for_values.last};{else},{/if}
{/foreach}
{/foreach}